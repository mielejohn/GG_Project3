...
public class PlayerFourScrit : MonoBehaviour {
	...
	public PlayerIndex playerIndexFour = PlayerIndex.Four;
	GamePadState state;
	GamePadState prevState;
	// Use this for initialization
	void Start()
	{
		playerIndexFour = PlayerIndex.Four;
	}
	...
	void FixedUpdate()
	{
		...
		playerIndexFour = PlayerIndex.Four;
		GamePad.SetVibration(playerIndexFour, state.Triggers.Left, state.Triggers.Right);
	}
	...
	void Update()
	{
		...
		if (!playerIndexSet || !prevState.IsConnected)
		{
			for (int i = 0; i < 4; ++i)
			{
				...
				if (testState.IsConnected)
				{
					...
					playerIndexFour = testPlayerIndex;
					playerIndexSet = true;
				}
			}
		}
		...
		prevState = state;
		state = GamePad.GetState(playerIndexFour);

		// Detect if a button was pressed this frame
		if (playerIndexFour == PlayerIndex.Four && prevState.Buttons.A == ButtonState.Released && state.Buttons.A == ButtonState.Pressed)
		{
		...
		if (playerIndexFour == PlayerIndex.Four && prevState.Buttons.A == ButtonState.Pressed && state.Buttons.A == ButtonState.Released)
		{
		...
		if (playerIndexFour == PlayerIndex.Four && prevState.Buttons.LeftShoulder == ButtonState.Released && state.Buttons.LeftShoulder == ButtonState.Pressed)
		{
			GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value, 1.0f);
		}

		if (playerIndexFour == PlayerIndex.Four && prevState.Buttons.LeftShoulder == ButtonState.Pressed && state.Buttons.LeftShoulder == ButtonState.Released)
		{
			GetComponent<Renderer>().material.color = new Color(255.0f, 1.0f, 1.0f, 1.0f);
		}
		// Make the current object turn

		if (playerIndexFour == PlayerIndex.One) {
