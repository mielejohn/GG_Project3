...
public class PlayerThreeScript : MonoBehaviour {
	...
	public PlayerIndex playerIndexThree = PlayerIndex.Three;
	GamePadState state;
	GamePadState prevState;
	// Use this for initialization
	void Start()
	{
		playerIndexThree = PlayerIndex.Three;
	}
	...
	void FixedUpdate()
	{
		...
		playerIndexThree = PlayerIndex.Three;
		GamePad.SetVibration(playerIndexThree, state.Triggers.Left, state.Triggers.Right);
	}
	...
	void Update()
	{
		...
		if (!playerIndexSet || !prevState.IsConnected)
		{
			for (int i = 0; i < 4; ++i)
			{
				...
				if (testState.IsConnected)
				{
					...
					playerIndexThree = testPlayerIndex;
					playerIndexSet = true;
				}
			}
		}
		...
		prevState = state;
		state = GamePad.GetState(playerIndexThree);

		// Detect if a button was pressed this frame
		if (playerIndexThree == PlayerIndex.Three && prevState.Buttons.A == ButtonState.Released && state.Buttons.A == ButtonState.Pressed)
		{
		...
		if (playerIndexThree == PlayerIndex.Three && prevState.Buttons.A == ButtonState.Pressed && state.Buttons.A == ButtonState.Released)
		{
		...
		if (playerIndexThree == PlayerIndex.Three && prevState.Buttons.LeftShoulder == ButtonState.Released && state.Buttons.LeftShoulder == ButtonState.Pressed)
		{
			GetComponent<Renderer>().material.color = new Color(Random.value, Random.value, Random.value, 1.0f);
		}

		if (playerIndexThree == PlayerIndex.Three && prevState.Buttons.LeftShoulder == ButtonState.Pressed && state.Buttons.LeftShoulder == ButtonState.Released)
		{
			GetComponent<Renderer>().material.color = new Color(255.0f, 1.0f, 1.0f, 1.0f);
		}
		// Make the current object turn

		if (playerIndexThree == PlayerIndex.One) {
